define([
  ...
  "dojo/request",
  "./Taxonomy",
], function (declare, Deferred, xhr, Taxonomy) {
    ...
    */
    ...
    buildTaxonIdByState: function (state) {
      ...
      if (taxon_id_or_name == parseInt(taxon_id_or_name)) {
        ...
        xhr
          .post(PathJoin(this.apiServiceUrl, "taxonomy"), {
            headers: {
            ...
            data:
              "eq(taxon_name," +
              taxon_id_or_name +
              ")&in(taxon_rank,(genus,species))&select(taxon_id,taxon_name,taxon_rank)&limit(1)",
            handleAs: "json",
          })
          .then(function (data) {
            if (data.length == 0) {
              def.reject(
                "Failed to load corresponding taxonomy: " + taxon_id_or_name
              );
            } else {
